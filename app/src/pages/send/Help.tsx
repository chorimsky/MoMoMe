...
export function Help({ support }: { support?: { email: string; phone: string } }) {
  ...
  return (
    <FlowCard>
      ...
      <p style={{ color: "var(--ink-2)", fontSize: 13.5, margin: "3px 0 12px" }}>{t("help_sub")}</p>
      <div>
        {FAQS.map((it, i) => {
          const on = open === i;
          return (
            <div key={it.q} style={{ borderBottom: "1px solid var(--line-2)" }}>
              <button onClick={() => setOpen(on ? -1 : i)} aria-expanded={on}
                style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "12px 2px", cursor: "pointer", border: "none", background: "transparent", font: "inherit", textAlign: "left" }}>
                <span style={{ fontWeight: 650, fontSize: 14, color: "var(--ink)" }}>{t(it.q)}</span>
              ...
              {on && <p style={{ fontSize: 13.5, color: "var(--ink-2)", lineHeight: 1.5, margin: "0 2px 12px" }}>{t(it.a)}</p>}
            </div>
          );
        })}
      </div>
