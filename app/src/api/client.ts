...
export const api = {
  ...
  platformTreasury: () => req<{ pool: string; currency: string; total: number | null; reserved: number; settlement_pending: number; available: number | null; float_unknown_reason: string[] | null }>("/admin/platform/treasury"),
  ...
  platformRequests: (all = false) => req<{ requests: Array<{ id: string; orgId: string; organization: string; requester: string; kind: string; status: string; createdAt: string; payload: Record<string, unknown>; decisionNote?: string; context: { kyb: string; plan: string; liveEnabled: boolean; status: string; country: string; credentials: number } | null; blocked: string | null }>; email_configured: boolean }>(`/admin/platform/requests${all ? "?all=1" : ""}`),
