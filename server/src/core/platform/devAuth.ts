...
export function verifyDevToken(token: unknown): { uid: string } | null {
  ...
  try { const d = JSON.parse(Buffer.from(payload, "base64url").toString()) as { uid?: string; exp?: number }; return d.uid && typeof d.exp === "number" && d.exp > Date.now() ? { uid: d.uid } : null; } catch { return null; }
}
