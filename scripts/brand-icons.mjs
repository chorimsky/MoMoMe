...
function compose(S, { share, bg, radius = 0, outline = 0, mono = false }) {
  const scale = (S * share) / BOX.h;
  const tx = S / 2 - (BOX.x + BOX.w / 2) * scale, ty = S / 2 - (BOX.y + BOX.h / 2) * scale;
  const body = mono
    ? `<g fill="${INK}"><path fill-rule="evenodd" d="M16 5a9 9 0 1 0 0 18a9 9 0 1 0 0-18zm0 3.8a5.2 5.2 0 1 1 0 10.4a5.2 5.2 0 0 1 0-10.4z"/><circle cx="16" cy="14" r="2.6"/><path d="M11 23.5 q5 4 10 0" fill="none" stroke="${INK}" stroke-width="2.2" stroke-linecap="round"/></g>`
    : mark;
