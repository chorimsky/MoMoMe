...
import { pruneIntents, allIntents, syncIntent, flagUnsettled } from "./core/upi/intents.js";
...
async function reconcileOnce(): Promise<void> {
  ...
  try { for (const i of allIntents(300)) if (i.refs.v1PaymentId && !["COMPLETED", "CANCELLED", "EXPIRED", "PAYMENT_FAILED", "SETTLEMENT_FAILED"].includes(i.state)) await syncIntent(i); for (const u of flagUnsettled()) console.warn(`[upi] ${u.id} (${u.refs.correlationId}) confirmed in but not settled — reconciliation required`); pruneIntents(); } catch (e) { console.error("upi intents", e); }
