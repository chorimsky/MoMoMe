...
async function main() {
  ...
  try {
    ...
    console.log("\n12. Account lifecycle: plan choice, verification, reset, invitations, logout everywhere, KYB → live access\n");
    const su2 = await dev("/signup", { email: "cto@fintech.example", name: "Chi", password: "twelve-char-pass", organization: "Fintech SA", plan: "business", expected_monthly_volume_xaf: "50000000" });
    ok("sign-up with plan=business starts on developer and opens a plan_change request; a verification link is issued (dev_link in sandbox)", su2.status === 201 && su2.body.organization.plan === "developer" && su2.body.plan_request?.kind === "plan_change" && /verify=vf_/.test(su2.body.email_verification?.dev_link ?? ""), JSON.stringify(su2.body.email_verification));
    const tok2 = su2.body.token as string; const org2 = su2.body.organization.id as string;
    ok("/me says the email is not verified yet and lists the plans", (await dev("/me", undefined, tok2)).body.user.emailVerified === false && (await dev("/me", undefined, tok2)).body.plans.length === 3);
    const vtoken = /verify=([^&]+)/.exec(su2.body.email_verification.dev_link)![1];
    ok("verify-email consumes the token once", (await dev("/verify-email", { token: vtoken })).body.ok === true && (await dev("/verify-email", { token: vtoken })).status === 400 && (await dev("/me", undefined, tok2)).body.user.emailVerified === true);
    const fp = await dev("/forgot-password", { email: "cto@fintech.example" });
    const fpNone = await dev("/forgot-password", { email: "nobody@fintech.example" });
    ok("forgot-password answers 200 either way and never reveals whether the address exists", fp.status === 200 && fpNone.status === 200 && !("dev_link" in fpNone.body) && /reset=rp_/.test(fp.body.dev_link ?? ""));
    const rtoken = /reset=([^&]+)/.exec(fp.body.dev_link)![1];
    const rs = await dev("/reset-password", { token: rtoken, password: "brand-new-password-1" });
    ok("reset-password sets the password, ends every earlier session and signs in fresh", rs.body.ok === true && rs.body.token && (await dev("/me", undefined, tok2)).status === 401 && (await dev("/login", { email: "cto@fintech.example", password: "brand-new-password-1" })).status === 200);
    const tok3 = rs.body.token as string;
    const cp = await dev("/password", { current: "brand-new-password-1", password: "yet-another-password-2" }, tok3);
    ok("change password requires the current one and rotates the session", cp.body.ok === true && (await dev("/me", undefined, tok3)).status === 401 && (await dev("/me", undefined, cp.body.token)).status === 200);
    const tok4 = cp.body.token as string;
    const inv = await dev(`/orgs/${org2}/members`, { email: "dev2@fintech.example", name: "Dee", role: "developer" }, tok4);
    ok("inviting a member issues an invitation link", inv.status === 201 && /invite=inv_/.test(inv.body.invitation?.dev_link ?? ""));
    const itoken = /invite=([^&]+)/.exec(inv.body.invitation.dev_link)![1];
    const peek = await dev(`/invitation/${itoken}`);
    ok("the invitation can be inspected before accepting", peek.body.email === "dev2@fintech.example" && peek.body.organization === "Fintech SA" && peek.body.needs_password === true);
    const acc = await dev(`/invitation/${itoken}/accept`, { password: "invited-user-pass-1" });
    ok("accepting sets the password and signs the member in with the developer role", acc.body.ok === true && (await dev("/me", undefined, acc.body.token)).body.organizations[0]?.role === "developer");
    const lo = await dev("/logout", { everywhere: true }, tok4);
    ok("logout everywhere invalidates the session server-side", lo.body.ok === true && (await dev("/me", undefined, tok4)).status === 401);
    const tok5 = (await dev("/login", { email: "cto@fintech.example", password: "yet-another-password-2" })).body.token as string;
    const la0 = await dev(`/orgs/${org2}/requests`, { kind: "live_access", note: "go live in October" }, tok5);
    ok("live access before KYB → 409 kyb_required", la0.status === 409 && la0.body.error === "kyb_required");
    const kyb0 = await dev(`/orgs/${org2}/requests`, { kind: "kyb", legal_name: "Fintech SA" }, tok5);
    ok("an incomplete KYB names the missing fields", kyb0.status === 400 && /registration_number/.test(kyb0.body.message));
    const kyb = await dev(`/orgs/${org2}/requests`, { kind: "kyb", legal_name: "Fintech SA", registration_number: "RC/DLA/2024/B/1234", country: "CM", contact_name: "Chi", address: "Douala", use_case: "payroll" }, tok5);
    ok("a complete KYB submission opens a request and marks KYB pending", kyb.status === 201 && kyb.body.kind === "kyb" && (await dev(`/orgs/${org2}`, undefined, tok5)).body.kyb === "pending");
    const la = await dev(`/orgs/${org2}/requests`, { kind: "live_access", note: "go live in October" }, tok5);
    ok("live access can be requested once KYB is submitted", la.status === 201);
    const queue = await (await fetch(`${base}/api/admin/platform/requests`, { headers: A })).json() as J;
    ok("Admin → activation queue lists the plan, KYB and live-access requests", ["plan_change", "kyb", "live_access"].every((k) => queue.requests.some((r: J) => r.orgId === org2 && r.kind === k)));
    for (const r of queue.requests.filter((r: J) => r.orgId === org2)) await fetch(`${base}/api/admin/platform/requests/${r.id}/approve`, { method: "POST", headers: A, body: JSON.stringify({ note: "welcome" }) });
    const o2 = (await dev(`/orgs/${org2}`, undefined, tok5)).body;
    ok("approving applies the change: business plan, KYB verified, live enabled", o2.plan.id === "business" && o2.kyb === "verified" && o2.liveEnabled === true, `${o2.plan?.id} ${o2.kyb} ${o2.liveEnabled}`);
    const liveCred = await dev(`/orgs/${org2}/credentials`, { environment: "live", label: "prod" }, tok5);
    ok("…and a live credential can now be minted", liveCred.status === 201 && /^mm_live_/.test(liveCred.body.secret));
    const mine = await dev(`/orgs/${org2}/requests`, undefined, tok5);
