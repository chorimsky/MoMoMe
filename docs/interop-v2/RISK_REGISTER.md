...
| 5 | Pricing real money on a configured FX rate | `fxLive()` refuses configured/stale/fallback rates when `liveMoney()`; `refreshPublicFx()` (Coinbase USD table, open.er-api fallback, 6 h max age, persisted) prices KES/GHS/NGN/UGX/TZS/RWF from a feed; the checklist shows the source per currency | — (open.er-api is the check venue; > 2 % apart = `divergent`, refused) |
