...
export interface Session { uid: string; role: AdminRole; elevatedUntil?: number; /** Password generation the token was issued under (see adminUsers.passwordVersion). */ pwv?: number; }
...
/** The server's general-purpose signing secret: the configured one, else a persisted random
 *  one. Shared with other internal integrity tokens (see recipientRisk) so none of them has
 *  to invent a fallback — a hard-coded default secret is a forgeable one. */
export const signingSecret = (): string => secret();
...
export function issueToken(session: Session): { token: string; expiresAt: string } {
  ...
  const payload = Buffer.from(JSON.stringify({ uid: session.uid, role: session.role, exp, ...(elv ? { elv } : {}), ...(session.pwv ? { pwv: session.pwv } : {}) })).toString("base64url");
...
export function verifyToken(token: unknown): Session | null {
  ...
  try {
    const d = JSON.parse(Buffer.from(payload, "base64url").toString()) as { uid?: string; role?: AdminRole; exp?: number; elv?: number; pwv?: number };
    ...
    return { uid: d.uid, role: d.role, ...(elevatedUntil ? { elevatedUntil } : {}), ...(typeof d.pwv === "number" ? { pwv: d.pwv } : {}) };
  } catch {
    return null;
  }
}
