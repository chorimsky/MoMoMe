...
import { assets, NETWORKS, SETTLEMENT_MODEL } from "../core/upi/assets.js";
...
api.get("/admin/identities", async (_req, res) => {
  ...
  // receivedXaf is HISTORY — what this number has actually been delivered. It is derived
  // here from delivered payouts rather than stored on the identity, so nothing can drift
  // into looking like a spendable balance.
  res.json(listIdentities().map((i) => ({ ...i, receivedXaf: receivedXaf.get(nsn(i.phone)) ?? 0 })));
});
