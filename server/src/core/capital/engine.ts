...
/** Test seam: drop the 10 s dataset cache. */
export function _resetCache(): void { cache = null; }
...
/** The one cost model (core/pricing.paymentCost): the payout cost recorded on the payment at
 *  delivery (invoice → contract → published → assumed) + crypto-rail cost + fixed overhead. */
export function costOf(p: Payment): number { return paymentCost(p).total; }
...
export async function revenue(f: IntelFilters = {}): Promise<IntelRevenue> {
  ...
  void c;
  const slice = (key: string, label: string, ps: Payment[]): RevenueSlice => { const volume = ps.reduce((s, p) => s + p.xaf, 0), fees = ps.reduce((s, p) => s + grossOf(p), 0); let provider = 0, settle = 0; for (const p of ps) { const k = paymentCost(p); provider += k.payout; settle += k.rail + k.fixed; } return { key, label, volume, customerFees: fees, providerCosts: provider, settlementCosts: settle, gross: fees, net: fees - provider - settle, marginPct: pct(fees - provider - settle, volume) }; };
...
export async function scanRecommendations(f: IntelFilters = {}): Promise<Recommendation[]> {
  ...
  // Margin: before telling anyone to reprice, say whether the loss is real or an artefact.
  const dg = diagnose(ds.completed);
  if (dg.payments >= 3 && dg.net < 0) {
    const assumedShare = dg.cost.payoutBySource.assumed.count / Math.max(1, dg.payments);
    if (assumedShare >= 0.5) mk({ type: "FIX_COST_MODEL", title: `The loss (${dg.net} XAF) rests on an assumed payout cost for ${Math.round(assumedShare * 100)} % of payments`, fingerprint: "COST:ASSUMED", inputs: [{ label: "Assumed payout cost", value: `${(dg.facts.assumedPayoutPct * 100).toFixed(2)} %`, source: SRC.pricing() }, { label: "Payments on the assumption", value: `${dg.cost.payoutBySource.assumed.count} of ${dg.payments}`, source: SRC.tx(ds) }, { label: "Assumed cost", value: `${dg.cost.payoutBySource.assumed.xaf} XAF`, source: SRC.pricing() }], calculation: { id: "COST-ASSUMED", formula: "Net = fees + spread − (payout cost + rail cost + fixed); payout cost = invoice → contract → published → ASSUMED", result: dg.net, steps: [{ label: "Fees", value: dg.revenue.fee, ccy: "XAF" }, { label: "Spread", value: dg.revenue.spread, ccy: "XAF" }, { label: "Payout cost (assumed part)", value: dg.cost.payoutBySource.assumed.xaf, ccy: "XAF" }, { label: "Payout cost (known part)", value: dg.cost.payout - dg.cost.payoutBySource.assumed.xaf, ccy: "XAF" }, { label: "Rail + fixed", value: dg.cost.rail + dg.cost.fixed, ccy: "XAF" }] }, output: `${dg.findings[1]?.detail ?? "Most of the cost side is a Settings figure, not an invoice."}`, recommendation: "Enter the signed PawaPay / Peexit disbursement schedules (Rates & Pricing → Contracted disbursement fees). Every report recomputes; only then decide on spreads or fees.", confidence: "HIGH", confidenceNote: "The share of assumed cost is measured; the true margin is unknown until the contracts are entered.", action: { kind: "PRICING", label: "Open Rates & Pricing", params: { href: "/admin" } }, expectedOutcome: "A margin figure built on invoices and contracts; the loss either disappears or is confirmed with a named cause.", risks: ["If the contracted fee is higher than assumed the loss is larger than shown."], responsible: null });
    const structural = dg.findings.find((f) => f.severity === "critical" && (f.title.startsWith("The fixed") || f.title.startsWith("Per XAF")));
    if (structural) mk({ type: "REPRICE", title: structural.title, fingerprint: "COST:STRUCTURAL", inputs: [{ label: "Average ticket", value: `${dg.facts.avgTicket} XAF`, source: SRC.tx(ds) }, { label: "Break-even ticket", value: dg.facts.breakEvenTicket == null ? "—" : `${dg.facts.breakEvenTicket} XAF`, source: SRC.pricing() }, { label: "Effective payout cost", value: `${dg.facts.effectiveCostPct ?? "—"} %`, source: SRC.pricing() }], calculation: { id: "COST-STRUCTURAL", formula: "Break-even ticket = (minimum fee − fixed cost) ÷ (payout% + rail% − spread%)", result: dg.facts.breakEvenTicket ?? 0, steps: [{ label: "Minimum fee", value: pricing().minFeeXaf, ccy: "XAF" }, { label: "Fixed cost / tx", value: pricing().costs.fixedXaf, ccy: "XAF" }, { label: "Spread % of volume", value: r2((dg.revenue.spread / Math.max(1, dg.volume)) * 100) }, { label: "Payout % of volume", value: dg.facts.effectiveCostPct ?? 0 }] }, output: structural.detail, recommendation: structural.action, confidence: "MEDIUM", confidenceNote: "Uses the period's own fee/spread mix; the cost side is only as good as the cost model.", action: { kind: "PRICING", label: "Open Rates & Pricing", params: { href: "/admin" } }, expectedOutcome: "Every ticket size clears its rail cost.", risks: ["A higher floor or wider spread reduces conversion on small tickets."], responsible: null });
  }
  for (const r of rt.routes.filter((x) => x.count >= 3 && (x.marginPct ?? 0) <= 0)) mk({ type: "REVIEW_ROUTE", title: `Route loses money: ${r.name}`, fingerprint: `ROUTE:${r.routeId}`, inputs: [{ label: "Revenue", value: `${r.revenue} XAF`, source: SRC.pricing() }, { label: "Cost", value: `${r.cost} XAF`, source: SRC.pricing() }], calculation: { id: `ROUTE-${r.routeId}`, formula: "Margin = revenue − cost", result: r.margin, steps: [{ label: "Revenue", value: r.revenue, ccy: "XAF" }, { label: "Cost", value: r.cost, ccy: "XAF" }] }, output: `${r.name} nets ${r.margin} XAF on ${r.count} payments.`, recommendation: "First make sure the cost side is real (contracted fees); then widen the spread on this rail or de-prioritise it in routing.", confidence: "MEDIUM", confidenceNote: "Costs are invoices/contracts where recorded on the payment, the assumption otherwise.", action: { kind: "PRICING", label: "Open pricing", params: { href: "/admin" } }, expectedOutcome: "Route margin ≥ 0.", risks: ["A wider spread reduces conversion."], responsible: null });
...
/* ---------- margin diagnosis: WHY is the number what it is ---------- */
export function diagnose(ps: Payment[]): MarginDiagnosis {
  const pr = pricing();
  const bySrc: MarginDiagnosis["cost"]["payoutBySource"] = { invoice: { count: 0, xaf: 0 }, contract: { count: 0, xaf: 0 }, published: { count: 0, xaf: 0 }, assumed: { count: 0, xaf: 0 } };
  let fee = 0, spread = 0, payout = 0, rail = 0, fixed = 0, volume = 0, noSpread = 0, atFloor = 0, merchantPaid = 0;
  const losers: MarginDiagnosis["losers"]["sample"] = []; let loserCount = 0, loserXaf = 0;
  for (const p of ps) {
    const k = paymentCost(p), sp = spreadOf(p), g = p.feeXaf + sp, net = g - k.total;
    fee += p.feeXaf; spread += sp; payout += k.payout; rail += k.rail; fixed += k.fixed; volume += p.xaf;
    bySrc[k.source].count++; bySrc[k.source].xaf += k.payout;
    if (!(typeof p.spreadBps === "number" && p.spreadBps > 0)) noSpread++;
    if (p.feeXaf <= pr.minFeeXaf && pr.minFeeXaf > 0) atFloor++;
    if (p.feeBy === "merchant") merchantPaid++;
    if (net < 0) {
      loserCount++; loserXaf += -net;
      const why = sp === 0 && !(typeof p.spreadBps === "number") ? "no spread recorded on this payment (older record) — revenue understated" : k.fixed > 0 && k.fixed >= g ? `the fixed per-transaction cost (${k.fixed} XAF) exceeds what a ${p.xaf} XAF ticket earns` : k.source === "assumed" ? `payout cost is the ${(pr.costs.payoutPct * 100).toFixed(2)} % assumption, above fee + spread on this rail` : "payout cost above fee + spread";
      if (losers.length < 8) losers.push({ ref: p.ref, xaf: p.xaf, fee: p.feeXaf, spread: sp, cost: k.total, net, why });
    }
  }
  const total = fee + spread, cost = payout + rail + fixed, net = total - cost;
  const avgTicket = ps.length ? r0(volume / ps.length) : 0;
  // Break-even ticket: fee%·x + spread·x + floor-effect ≥ payoutPct·x + railPct·x + fixed. With the floor,
  // small tickets earn max(fee%·x, floor); solve for x where floor applies: floor + s·x = c·x + fixed.
  const spreadRate = volume > 0 ? spread / volume : 0;
  const payoutRate = volume > 0 ? payout / volume : pr.costs.payoutPct;
  const perXaf = spreadRate - payoutRate - pr.costs.railPct;
  const breakEven = perXaf >= 0 ? 0 : pr.minFeeXaf - pr.costs.fixedXaf > 0 ? r0((pr.minFeeXaf - pr.costs.fixedXaf) / -perXaf) : null;
  const effectiveCostPct = volume > 0 ? r2((payout / volume) * 100) : null;
  const findings: MarginDiagnosis["findings"] = [];
  if (net < 0) findings.push({ severity: "critical", title: `The period nets ${r0(net)} XAF on ${ps.length} payments`, detail: `Revenue ${r0(total)} XAF (fees ${r0(fee)}, spread ${r0(spread)}) against costs ${r0(cost)} XAF (payout ${r0(payout)}, crypto rail ${r0(rail)}, fixed ${r0(fixed)}).`, action: "Read the findings below in order — the first one is the largest cause." , impactXaf: r0(-net) });
  if (bySrc.assumed.count > 0 && bySrc.assumed.count >= ps.length * 0.5) findings.push({ severity: net < 0 ? "critical" : "warning", title: `${Math.round((bySrc.assumed.count / Math.max(1, ps.length)) * 100)} % of payout costs are the ${(pr.costs.payoutPct * 100).toFixed(2)} % ASSUMPTION, not an invoice or contract`, detail: `Only Peexit publishes its fee over the API; PawaPay's is whatever Settings → Pricing says. If the real schedule is lower, the "loss" is partly an accounting artefact; if higher, it is worse than shown.`, action: "Enter the signed PawaPay/Peexit schedule under Rates & Pricing → Contracted disbursement fees; the figure recomputes on every report.", impactXaf: r0(bySrc.assumed.xaf) });
  if (noSpread > 0) findings.push({ severity: "warning", title: `${noSpread} payment(s) carry no spread — their FX revenue counts as zero`, detail: "Payments created before the spread was recorded at quote time show fee-only revenue here, which drags the margin below what was actually earned.", action: "Judge the margin on payments from the last 30 days, which all carry it; the older records cannot be repriced." });
  if (pr.costs.fixedXaf > 0 && perXaf < 0 && (breakEven == null || breakEven > avgTicket)) findings.push({ severity: "critical", title: breakEven == null ? `The fixed ${pr.costs.fixedXaf} XAF per-transaction cost exceeds the ${pr.minFeeXaf} XAF minimum fee — no ticket size is profitable at this cost model` : `The fixed ${pr.costs.fixedXaf} XAF per-transaction cost makes every ticket under ≈ ${breakEven.toLocaleString("en")} XAF a loss`, detail: `Average ticket this period is ${avgTicket.toLocaleString("en")} XAF. The fixed cost is a modelled overhead, not something a rail bills per payout.`, action: "Set the fixed cost to what is actually paid per payout (often 0) or raise the minimum fee to cover it." });
  if (perXaf < 0 && pr.costs.fixedXaf === 0 && bySrc.assumed.count === ps.length && ps.length > 0) findings.push({ severity: "critical", title: `Per XAF, spread (${(spreadRate * 100).toFixed(2)} %) covers less than the modelled payout + rail cost (${((payoutRate + pr.costs.railPct) * 100).toFixed(2)} %)`, detail: "Only the fee floor keeps small tickets positive; large tickets lose money at the modelled cost.", action: "Either the cost model is too high (enter contracts) or the spread must widen on the losing rail — Rates & Pricing → spreads." });
  if (atFloor > ps.length * 0.6 && ps.length > 0) findings.push({ severity: "info", title: `${atFloor} of ${ps.length} payments hit the ${pr.minFeeXaf} XAF minimum fee`, detail: "Most tickets are small; the floor is doing the work of the percentage.", action: "Keep the floor ≥ the rails' own minimums; consider a slightly higher floor for tickets under 5 000 XAF." });
  if (merchantPaid > 0) findings.push({ severity: "info", title: `${merchantPaid} merchant-paid checkout(s)`, detail: "The business carried the fee — same revenue to the platform, better price for the customer.", action: "Grow this share: it is the highest-margin consumer product." });
  if (losers.length) findings.push({ severity: net < 0 ? "warning" : "info", title: `${loserCount} payment(s) lost money individually (${r0(loserXaf)} XAF)`, detail: losers.slice(0, 3).map((l) => `${l.ref}: ${l.xaf} XAF, net ${l.net} — ${l.why}`).join(" · "), action: "The reasons name the lever: spread, floor, fixed cost or the cost assumption." });
  if (!findings.length) findings.push({ severity: "info", title: "Margin is positive and every component is explained", detail: `Effective payout cost ${effectiveCostPct ?? "—"} % · spread ${(spreadRate * 100).toFixed(2)} % · fee ${(volume ? (fee / volume) * 100 : 0).toFixed(2)} %.`, action: "Enter contracted fees to turn the assumed part into fact." });
  return { payments: ps.length, volume, revenue: { fee: r0(fee), spread: r0(spread), total: r0(total) }, cost: { payout: r0(payout), rail: r0(rail), fixed: r0(fixed), total: r0(cost), payoutBySource: bySrc }, net: r0(net), marginPct: pct(net, volume), losers: { count: loserCount, xaf: r0(loserXaf), sample: losers }, facts: { noSpreadRecorded: noSpread, feeAtFloor: atFloor, merchantPaid, avgTicket, breakEvenTicket: breakEven, assumedPayoutPct: pr.costs.payoutPct, effectiveCostPct }, findings };
}
