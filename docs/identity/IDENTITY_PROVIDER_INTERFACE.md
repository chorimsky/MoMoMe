...
| `peexit_verify` | yes | `MTN` and `ORANGE` × CM | Peexit **Get KYC** `POST /clients/verify_wallet` `{ countryCode: "cm", accountNumber }` → `{ valid, accountTitle, accountStatus, accountType }` (docs: peex-api-docs.peexit.com/get-kyc — the route deployed on production); falls back once to `/clients/verify-wallet` `{ isValid, accountName, operator, status }` (the sandbox "Verify Wallet" page) when the first is a route-404 | same `SECRETKEY` and IP allowlist as disbursement, so it answers only from Railway; 404 → NOT_FOUND; `isValid:false` → INACTIVE; 200 without `accountName` → UNKNOWN (never a fabricated name); 401/403 → non-retryable AUTH_ERROR; 422 → UNSUPPORTED_COUNTRY |
