...
import { MARKETS, corridorId } from "../core/network/markets.js";
...
network.post("/intents", rateLimitDurableMiddleware("network_intents", 30, 60_000), async (req, res) => {
  const owner = (req as unknown as { senderId?: string }).senderId ?? String(req.headers["x-mm-sender"] ?? "");
  const b = (req.body ?? {}) as Record<string, unknown>;
  const src = MARKETS[String(b.sourceMarket ?? "")], dst = MARKETS[String(b.destinationMarket ?? "")];
  ...
  const intent: NetworkIntent = {
    id: id("nin"), owner: owner || "anonymous",
...
network.post("/intents/:id/confirm", rateLimitDurableMiddleware("network_confirm", 30, 60_000), async (req, res) => {
  ...
  const owner = (req as unknown as { senderId?: string }).senderId ?? String(req.headers["x-mm-sender"] ?? "");
  if (intent.owner !== (owner || "anonymous")) return res.status(403).json({ error: "forbidden", message: "Not your intent." });
  ...
  if (!r.ok) return res.status(r.error === "quote_expired" ? 409 : r.error === "network_off" ? 403 : 409).json({ error: r.error, message: r.message });
  res.json({ transaction: r.tx });
});

network.get("/transactions/:id", (req, res) => {
  ...
  if (intent && intent.owner !== (owner || "anonymous")) return res.status(404).json({ error: "not_found", message: "Transaction not found." });
...
export async function overview(): Promise<NetworkOverview> {
  ...
  return {
    flags: s.flags,
    markets: Object.values(MARKETS).map((m) => ({ ...m, corridorsOut: corridors.filter((c) => c.source === m.code && c.status === "ACTIVE").length, liquidityAvailable: positions.filter((p) => p.market === m.code && p.available != null).reduce<number | null>((a, p) => (a ?? 0) + (p.available ?? 0), null) })),
